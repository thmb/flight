...
create_destination "${workspace_id}" "${inherit}" \
	"./destinations/postgres/local.json" postgres_id

echo ""

create_connection "${workspace_id}" "${mes_id}" "${postgres_id}" "${normalization_id}" \
	"./connections/production/mes.json"

create_connection "${workspace_id}" "${stor_id}" "${postgres_id}" "${normalization_id}" \
	"./connections/purchase/sap.json"
create_connection "${workspace_id}" "${stor_id}" "${postgres_id}" "${normalization_id}" \
	"./connections/purchase/stor.json"

create_connection "${workspace_id}" "${salesforce_id}" "${postgres_id}" "${normalization_id}" \
	"./connections/sales/salesforce.json"
create_connection "${workspace_id}" "${sap_id}" "${postgres_id}" "${normalization_id}" \
